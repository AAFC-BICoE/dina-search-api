...
for currIndex in ${index_array[@]}; do
  ...
  CURRENT_INDEX_NAME=$(./wait-for-elasticsearch.sh $ELASTIC_SERVER_URL "curl -s -u elastic:$ES_PASSWORD --cacert $ES_CA_FILE -X GET "$ELASTIC_SERVER_URL/_alias/${!indexName}" | jq -r 'keys[0]'")

  if [[ "$CURRENT_INDEX_NAME" != "${!indexName}"* ]]; then
    response="$(curl -s -o /dev/null -u elastic:$ES_PASSWORD --cacert $ES_CA_FILE -I -w "%{http_code}" "$ELASTIC_SERVER_URL/${!indexName}")"
